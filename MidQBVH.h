#ifndef INC_LRENDER_MIDQBVH_H__
#define INC_LRENDER_MIDQBVH_H__
/**
@file MidQBVH.h
@author t-sakai
...
namespace lrender
{
    ...
    private:
        MidQBVH(const MidQBVH&);
        MidQBVH& operator=(const MidQBVH&);
    ...
    const f32 MidQBVH<PrimitiveType, PrimitivePolicy>::Epsilon = 1.0e-6f;

    template<class PrimitiveType, class PrimitivePolicy>
    void MidQBVH<PrimitiveType, PrimitivePolicy>::Node::setBBox(const BBox bbox[4])
    {
    ...
    template<class PrimitiveType, class PrimitivePolicy>
    void MidQBVH<PrimitiveType, PrimitivePolicy>::Node::clear()
    {
    ...
    template<class PrimitiveType, class PrimitivePolicy>
    MidQBVH<PrimitiveType, PrimitivePolicy>::MidQBVH()
        :primitives_(NULL)
        ,depth_(0)
        ,stack_(NULL)
    {
    ...
    template<class PrimitiveType, class PrimitivePolicy>
    void MidQBVH<PrimitiveType, PrimitivePolicy>::build(s32 numPrimitives, const PrimitiveType* primitives)
    {
    ...
    template<class PrimitiveType, class PrimitivePolicy>
    inline void MidQBVH<PrimitiveType, PrimitivePolicy>::getBBox(BBox& bbox, s32 start, s32 end)
    {
        bbox.setInvalid();
    ...
    template<class PrimitiveType, class PrimitivePolicy>
    void MidQBVH<PrimitiveType, PrimitivePolicy>::recursiveConstruct(s32 start, s32 numPrimitives, s32 nodeIndex, const BBox& bbox, s32 depth)
    {
        BBox childBBox[4];
        s32 primStart[4];
        s32 num[4];
        {
            ...
            //left分割
            splitMid(node.axis1_, num[0], num[1], childBBox[0], childBBox[1], start, num[0], childBBox[0]);
            ...
            nodes_[nodeIndex].setBBox(childBBox);
            for(s32 i=0; i<4; ++i){
    ...
    template<class PrimitiveType, class PrimitivePolicy>
    void MidQBVH<PrimitiveType, PrimitivePolicy>::splitMid(s32& axis, s32& num_l, s32& num_r, BBox& bbox_l, BBox& bbox_r, s32 start, s32 numPrimitives, const BBox& bbox)
    {
        //最大の軸を半分に分割
    ...
    template<class PrimitiveType, class PrimitivePolicy>
    HitRecord MidQBVH<PrimitiveType, PrimitivePolicy>::intersect(Ray& ray)
    {
        __m128 origin[3];
        __m128 invDir[3];
        __m128 tminSSE;
    ...
    template<class PrimitiveType, class PrimitivePolicy>
    void MidQBVH<PrimitiveType, PrimitivePolicy>::print(const char* filename)
    {
