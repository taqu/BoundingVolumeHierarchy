#ifndef INC_LRENDER_BVH_H__
#define INC_LRENDER_BVH_H__
/**
@file BVH.h
@author t-sakai
...
namespace lrender
{
    ...
    const f32 BVH<PrimitiveType, PrimitivePolicy>::Epsilon = 1.0e-6f;

    template<class PrimitiveType, class PrimitivePolicy>
    BVH<PrimitiveType, PrimitivePolicy>::BVH()
        :SAH_KI_(1.5f)
        ,SAH_KT_(1.0f)
        ,primitives_(NULL)
        ,depth_(0)
    ...
    template<class PrimitiveType, class PrimitivePolicy>
    void BVH<PrimitiveType, PrimitivePolicy>::build(s32 numPrimitives, const PrimitiveType* primitives)
    {
    ...
    template<class PrimitiveType, class PrimitivePolicy>
    inline void BVH<PrimitiveType, PrimitivePolicy>::getBBox(BBox& bbox, s32 start, s32 end)
    {
        bbox.setInvalid();
    ...
    template<class PrimitiveType, class PrimitivePolicy>
    void BVH<PrimitiveType, PrimitivePolicy>::recursiveConstruct(s32 start, s32 numPrimitives, s32 nodeIndex, s32 depth)
    {
        ...
        f32 area_l, area_r;
        s32 axis = 0;
        if(area<=Epsilon || numPrimitives<=2){
            ...
            for(s32 curAxis=0; curAxis<3; ++curAxis){
                ...
                for(s32 m=start+1; m<end; ++m){
                    ...
                    f32 cost = SAH_KT_ + SAH_KI_*(area_l*invArea*num_l + area_r*invArea*num_r);
                    if(cost<bestCost){
                        mid = m;
                        bestCost = cost;
                        axis = curAxis;
    ...
    template<class PrimitiveType, class PrimitivePolicy>
    HitRecord BVH<PrimitiveType, PrimitivePolicy>::intersect(Ray& ray)
    {
        HitRecord hitRecord;
        hitRecord.t_ = ray.t_;
    ...
    template<class PrimitiveType, class PrimitivePolicy>
    void BVH<PrimitiveType, PrimitivePolicy>::print(const char* filename)
    {
